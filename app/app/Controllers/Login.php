...
class Login extends DefaultController {
    ...
    protected function doSociallogin() 
    {
        ...
        if (!$this->defmodel->checkSocialLoginConfigured($network)) {
            $msg = $this->_('socialnetworknotconfigured','account',$network);
            throw new DoException($this->makeUrl(array('message'=>'e:'.$msg)),$msg,'login',403,'redirect');
        }
    ...
    protected function doActivationresend() 
    {
        ...
        try {
            ...
            // model will return Exception, but we need extended exception with a redirect url
            throw new DoException(
